...
import (
	...
	"stormfront-cli/restart/application"
	"stormfront-cli/restart/client"
	"stormfront-cli/utils"
)

var RestartHelpText = fmt.Sprintf(`usage: stormfront restart <command> [-l|--log-level <log level>] [-h|--help]
commands:
	application       Restart an existing application
	client            Restart a Stormfront client 
arguments:
	-l|--log-level    Sets the log level of the CLI. valid levels are: %s, defaults to %s
	-h|--help         Show this help message and exit`, logging.GetDefaults(), logging.ERROR_NAME)

func ParseRestartArgs(args []string) {
