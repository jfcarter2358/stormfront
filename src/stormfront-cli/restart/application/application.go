...
var ApplicationHelpText = fmt.Sprintf(`usage: stormfront restart application <application id> [-H|--host <stormfront host>] [-p|--port <stormfront port>] [-l|--log-level <log level>] [-h|--help]
arguments:
...
func ExecuteApplication(host, port, id string) error {
	...
	requestURL := fmt.Sprintf("http://%s:%s/api/application/%s/restart", host, port, id)
