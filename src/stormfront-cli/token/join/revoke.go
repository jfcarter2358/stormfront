...
var APITokenRevokeHelpText = fmt.Sprintf(`usage: stormfront token join revoke -t <token> [-H|--host <stormfront host>] [-p|--port <stormfront port>] [-l|--log-level <log level>] [-h|--help]
arguments:
	-t|--token        The join token to revoke
...
func ExecuteRevoke(token, host, port string) error {
	logging.Info("Revoking join token...")

	requestURL := fmt.Sprintf("http://%s:%s/auth/join/%s", host, port, token)
	...
	if resp.StatusCode == http.StatusOK {
		logging.Success("Join token has been revoked")
	} else {
