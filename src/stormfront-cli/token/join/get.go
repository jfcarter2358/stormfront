...
var APITokenGetHelpText = fmt.Sprintf(`usage: stormfront token join get [-H|--host <stormfront host>] [-p|--port <stormfront port>] [-l|--log-level <log level>] [-h|--help]
arguments:
...
func ExecuteGet(host, port string) error {
	...
	requestURL := fmt.Sprintf("http://%s:%s/auth/join", host, port)
