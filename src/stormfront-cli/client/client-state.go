...
var ClientStateHelpText = fmt.Sprintf(`usage: stormfront client state [-H|--host <stormfront host>] [-p|--port <stormfront port>] [-l|--log-level <log level>] [-h|--help]
arguments:
...
func ExecuteState(host, port string) error {
	logging.Info("Getting stormfront client health...")

	requestURL := fmt.Sprintf("http://%s:%s/api/state", host, port)
